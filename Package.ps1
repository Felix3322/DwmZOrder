...
if ($receipt.configuration -ne 'Release' -or !$receipt.buildVerified -or $receipt.inputs.Count -ne $expectedInputs.Count) { throw 'A successful Release build from this source is required.' }
...
foreach ($file in @('README.md','README.en.md','LICENSE','NOTICE.md','SUPPORT.md','source-manifest.json')) { Copy-Item -LiteralPath (Join-Path $root $file) -Destination $portable }
