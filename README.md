...
2. 实际调整需要管理员权限。点击“以管理员身份重新启动”，或以管理员身份运行 EXE。
3. 选择目标窗口；需要相对排序时再选择参照窗口。位置支持最前方、最后方、参照窗口上方和参照窗口下方。
4. 点击“应用顺序”。勾选“持续保持此顺序”后，DWM 更新场景时会继续维护该目标。用“恢复目标”恢复 Windows 当前顺序，用“停止全部保持”停止本会话代理的保持。

“读取顺序”仅查看当前顺序，不移动窗口。结果会显示目标名称、它在 DWM 列表中的位置，以及持续保持是否开启。第 1 位表示最前方；列表总项数不等于屏幕上可见窗口的数量。结果文字按 Windows 界面语言显示中文或英文。
